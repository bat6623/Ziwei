...
export const InfoModals: React.FC<InfoModalsProps> = ({ activeTab, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[55] bg-black/40 backdrop-blur-sm flex items-end sm:items-center justify-center p-2 sm:p-4 font-apple animate-fade-in" onClick={onClose}>
      <div
        className="bg-card w-full sm:max-w-lg rounded-[28px] max-h-[88vh] overflow-y-auto pb-[max(1rem,env(safe-area-inset-bottom))]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="sticky top-0 bg-card/90 backdrop-blur-xl flex items-center gap-3 h-[72px] px-5">
          <span className="flex-1 text-[28px] font-light tracking-tight text-label">{activeTab === 'help' ? '使用說明' : '關於'}</span>
          <button type="button" onClick={onClose} aria-label="關閉" className="w-10 h-10 rounded-full border border-separator text-label flex items-center justify-center hover:bg-fill active:bg-fill2 cursor-pointer">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {activeTab === 'help' && (
          <div className="px-4 pb-2">
            <ul className="bg-grouped rounded-[20px] overflow-hidden divide-y divide-separator">
              {HELP_ITEMS.map((item) => (
        ...
        {activeTab === 'about' && (
          <div className="px-4 pb-2 space-y-5">
            ...
            <div>
              <h5 className="px-4 mb-1.5 text-[13px] text-label3">計算依據</h5>
              <ul className="bg-grouped rounded-[20px] overflow-hidden divide-y divide-separator text-[15px] text-label2">
